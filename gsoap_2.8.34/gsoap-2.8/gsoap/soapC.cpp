...
SOAP_SOURCE_STAMP("@(#) soapC.cpp ver 2.8.34 2017-11-23 10:56:01 GMT")
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag_c && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_int(soap, "c", &a->c, "xsd:int"))
				{	soap_flag_c--;
					continue;
				}
			if (soap->error == SOAP_TAG_MISMATCH)
		...
		if (soap_element_end_in(soap, tag))
			return NULL;
		if ((soap->mode & SOAP_XML_STRICT) && (soap_flag_c > 0))
		{	soap->error = SOAP_OCCURS;
			return NULL;
		}
	}
